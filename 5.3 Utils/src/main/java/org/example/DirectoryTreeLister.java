    ...
    private void listDirectoryTree(File dir, int level) throws IOException {
        try {
            ...
            if (items != null) {
                ...
                for (File item : items) {
                    if (item.isDirectory()) {
                        writer.write("  ".repeat(level) + "D: " + item.getName() + " - Last modified: " + getModificationDate(item) + "\n");
                        listDirectoryTree(item, level + 1);
                    } else {
                        writer.write("  ".repeat(level) + "F: " + item.getName() + " - Last modified: " + getModificationDate(item) + "\n");
                    }
                }
            }
